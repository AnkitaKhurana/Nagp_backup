...
export class Producer {
    ...
    public Publish = async (msg: any) => {
        return this.SetupMessaging()
            .then((ok) => {
                console.log('[ * ] publishing to exchange ');
                let published = this._channel.publish(constants.USER_FANOUT_EXCHANGE, '', Buffer.from(JSON.stringify(msg)));
                if(published) { 
    ...
    private SetupMessaging() {
        return this.CreateConnection()
                ...
                .then((ch: amqp.Channel) => this.CreateExchange())
    }
