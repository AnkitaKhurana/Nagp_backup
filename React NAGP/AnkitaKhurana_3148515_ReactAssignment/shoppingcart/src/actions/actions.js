import {
    ...
    showOrderPlacedDialogBox, fetchProductsActionWithoutPagination
} from './types';
...
export const fetchProductsActionWithoutPaginationAction = (name, sort) => (dispatch) => {
    if (name === undefined)
        name = "";
    if (sort === undefined)
        sort = "";   //asc|desc
    fetch('http://localhost:3004/products?name_like=' + name + '&_sort=price,discount&_order=' + sort + ',' + sort)
        .then(res => res.json())
        .then(data => dispatch({
            type: fetchProductsActionWithoutPagination,
            result: data
        }));
};
